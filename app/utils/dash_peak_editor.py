...
The peak-to-peak amplitude (|max_val - min_val|) is displayed inside the Dash
app and updates live on every cursor drag, without needing a Streamlit rerun.
"""
...
def _amplitude_str(points: dict) -> str:
    """
    Compute peak-to-peak amplitude from the current points store.
    points = {"x": [x_min, x_max], "y": [y_min, y_max]}
    """
    ys = points.get("y", []) if points else []
    if len(ys) < 2 or ys[0] is None or ys[1] is None:
        return "Peak-to-peak: —"
    amp = abs(float(ys[1]) - float(ys[0]))
    return f"Peak-to-peak: {amp:.1f} µV"
...
def _ctx_from_request() -> tuple[dict, list[str]]:
    ...
    if not block and referer:
        try:
            ...
            dbg.append(f"referer_parse_error={e}")
...
def _cursor_shapes(x_min: float, x_max: float) -> list[dict]:
    def _one(x):
        return dict(
            ...
            y0=-0.05, y1=1.05, xref="x", yref="paper",
            line=dict(width=2, dash="solid"), layer="above",
        )
...
def _epoch_shapes(epoch) -> list[dict]:
    return [
        dict(type="line", x0=float(epoch.tmin_ms), x1=float(epoch.tmin_ms),
             y0=-0.05, y1=1.05, xref="x", yref="paper",
             line=dict(width=1, dash="dash"), layer="above"),
        dict(type="line", x0=float(epoch.tmax_ms), x1=float(epoch.tmax_ms),
             y0=-0.05, y1=1.05, xref="x", yref="paper",
             line=dict(width=1, dash="dash"), layer="above"),
    ]
...
def make_figure(
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=t_ms, y=y, mode="lines", name="MEP", hoverinfo="skip"))
    fig.add_trace(go.Scatter(
        x=[x0, x1], y=[float(points["y"][0]), float(points["y"][1])],
        mode="markers", name="Min/Max",
        marker=dict(size=16), hoverinfo="skip",
    ))
...
def _extract_cursor_x_from_relayout(
    relayout: dict, fallback_points: dict
    ...
    if x0 is not None or x1 is not None:
        ...
        return (x0 if x0 is not None else prev0,
                x1 if x1 is not None else prev1)

    if ("shapes" in relayout
            and isinstance(relayout["shapes"], list)
            and len(relayout["shapes"]) >= 2):
        try:
...
def _assign_min_max(
    ...
    """Assign min/max by signal amplitude, not cursor order."""
    if ya <= yb:
        return [xa, ya], [xb, yb]
    else:
        return [xb, yb], [xa, ya]
...
"""
    ...
    def _serve_layout():
        ...
        if not ctx:
            ...
            try:
                ...
                t_ms, meps = load_meps_for_block(
                    meta, session_file,
                    block_name=block, hemi=hemi,
                    epoch=epoch, detrend=False,
                )
                ...
                if not isinstance(meps, np.ndarray) or meps.ndim != 2 or meps.size == 0:
                    ...
                    fig = _error_figure(f"MEP index out of range: {i}")
                else:
                    y = meps[i, :]
                    ...
                    x0 = (
                        float(stored_min[0])
                        if isinstance(stored_min, (list, tuple)) and len(stored_min) == 2
                           and stored_min[0] is not None
                        else float(epoch.tmin_ms)
                    )
                    x1 = (
                        float(stored_max[0])
                        if isinstance(stored_max, (list, tuple)) and len(stored_max) == 2
                           and stored_max[0] is not None
                    ...
                    amp_text  = _amplitude_str(points)
                    fig       = make_figure(t_ms, y, points, epoch,
                                            f"{block} / {hemi} / MEP {i}")
        ...
        # Live amplitude display — updated by on_drag callback.
        children.append(
            html.Div(
    ...
    @app.callback(
        ...
        Output("graph",             "figure"),
        Output("amplitude-display", "children"),   # live update on drag
    ...
    def on_drag(relayout_data, points, ctx):
        ...
        t_ms, meps = load_meps_for_block(
            meta, session_file,
            block_name=ctx["block"], hemi=ctx["hemi"],
            epoch=epoch, detrend=False,
        )
        ...
        fig      = make_figure(t_ms, y, new_points, epoch,
                               f"{block} / {hemi} / MEP {i}")
        amp_text = _amplitude_str(new_points)

        return new_points, fig, amp_text
